...
class NameGen:
	...
	def __init__(self, language_file, random_gen = None, forbidden_file = None):
		self.min_syl = 2
		self.max_syl = 4
                self.random = random_gen or random
	...
	def gen_word(self, no_repeat = False):
		...
		num_syl = self.random.randint(self.min_syl, self.max_syl - 1)
		...
		while len(word) < self.min_syl or self.forbidden.find(word_str) != -1:
			...
			syl = _select_syllable(self.starts, 0, self.random)
			...
			for i in range(1, num_syl):
				...
				syl = _select_syllable(self.combinations[syl], end, self.random)
				...
				syl = _select_syllable(self.ends, 0, self.random)
				word.append(self.syllables[syl])
...
def _select_syllable(counts, end_count, random):
